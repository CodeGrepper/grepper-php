...
class ApiRequestor
{
    ...
    public function __construct($apiKey = null, $apiBase = null)
    {
        $this->_apiKey = $apiKey;
        if (!$apiBase) {
            $apiBase = Grepper::$apiBase;
        }
        $this->_apiBase = $apiBase;
    }

    /**
     * Creates a telemetry json blob for use in 'X-Grepper-Client-Telemetry' headers.
     *
     * @static
     *
    ...
    private static function _telemetryJson($requestTelemetry)
    {
        ...
        Grepper::getLogger()->error('Serializing telemetry payload failed!');
    ...
    {
        $uaString = 'Grepper/v1 PhpBindings/' . Grepper::VERSION;

        $langVersion = \PHP_VERSION;
        $uname_disabled = static::_isDisabled(\ini_get('disable_functions'), 'php_uname');
        $uname = $uname_disabled ? '(disabled)' : \php_uname();

        $appInfo = Grepper::getAppInfo();
        $ua = [
            'bindings_version' => Grepper::VERSION,
            'lang' => 'php',
        ...
        return [
            'X-Grepper-Client-User-Agent' => \json_encode($ua),
            'User-Agent' => $uaString,
    ...
    private function _prepareRequest($method, $url, $params, $headers)
    {
        ...
        if (!$myApiKey) {
            $myApiKey = Grepper::$apiKey;
        }

        if (!$myApiKey) {
            $msg = 'No API key provided.  (HINT: set your API key using '
              . '"Grepper::setApiKey(<API-KEY>)".  You can generate API keys from '
              . 'the Grepper web interface.  See https://stripe.com/api for '
        ...
        // X-Grepper-Client-User-Agent header via the optional getUserAgentInfo()
        // method
        $clientUAInfo = null;
        ...
        if (Grepper::$apiVersion) {
            $defaultHeaders['Grepper-Version'] = Grepper::$apiVersion;
        }

        if (Grepper::$accountId) {
            $defaultHeaders['Grepper-Account'] = Grepper::$accountId;
        }

        if (Grepper::$enableTelemetry && null !== self::$requestTelemetry) {
            $defaultHeaders['X-Grepper-Client-Telemetry'] = self::_telemetryJson(self::$requestTelemetry);
        }
