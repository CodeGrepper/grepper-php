...
namespace Grepper\Util;

use Grepper\GrepperObject;
...
abstract class Util
{
    ...
    /**
     * Converts a response from the Grepper API to the corresponding PHP object.
     *
     * @param array $resp the response from the Grepper API
     * @param array $opts
     *
     * @return array|GrepperObject
     */
    public static function convertToGrepperObject($resp, $opts)
    {
        $types = \Grepper\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
                $mapped[] = self::convertToGrepperObject($i, $opts);
            }
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \Grepper\GrepperObject::class;
            }
    ...
    {
        if ($h instanceof \Grepper\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
