...
class RequestOptions
{
    ...
    public static function parse($options, $strict = false)
    {
        ...
        if (\is_array($options)) {
            ...
            if (\array_key_exists('grepper_account', $options)) {
                $headers['Grepper-Account'] = $options['grepper_account'];
                unset($options['grepper_account']);
            }
            if (\array_key_exists('grepper_version', $options)) {
                $headers['Grepper-Version'] = $options['grepper_version'];
                unset($options['grepper_version']);
            }
        ...
        $message = 'The second argument to Grepper API method calls is an '
           . 'optional per-request apiKey, which must be a string, or '
           . 'per-request options, which must be an array. (HINT: you can set '
           . 'a global apiKey by "Grepper::setApiKey(<apiKey>)")';
