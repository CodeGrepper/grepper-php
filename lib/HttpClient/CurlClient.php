...
namespace Grepper\HttpClient;

use Grepper\Exception;
use Grepper\Grepper;
use Grepper\Util;
...
class CurlClient implements ClientInterface, StreamingClientInterface
{
    ...
    /** @var \Grepper\Util\RandomGenerator */
     ...
     * @param null|\Grepper\Util\RandomGenerator $randomGenerator
     */
     ...
     *   <li>\Grepper\Util\CaseInsensitiveArray $rheaders The response headers</li>
    ...
    private function constructRequest($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (('post' === $method) && (Grepper::$maxNetworkRetries > 0)) {
        ...
        $opts[\CURLOPT_CAINFO] = Grepper::getCABundlePath();
        if (!Grepper::getVerifySslCerts()) {
        ...
        // force IPv4 resolving as Grepper's API servers are only accessible over
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case \CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Grepper ({$url}).  Please check your "
                 . 'internet connection and try again.  If this problem persists, '
                 . "you should check Grepper's service status at "
            ...
            case \CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Grepper's SSL certificate.  Please make sure "
            ...
            default:
                $msg = 'Unexpected error communicating with Grepper.  '
                 . 'If this problem persists,';
        }
     ...
     * @param int $errno
     * @param int $rcode
     * @param array|\Grepper\Util\CaseInsensitiveArray $rheaders
     * @param int $numRetries
     *
     * @return bool
     */
    ...
    {
        if ($numRetries >= Grepper::getMaxNetworkRetries()) {
            return false;
        }
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = \min(
            Grepper::getInitialNetworkRetryDelay() * 1.0 * 2 ** ($numRetries - 1),
            Grepper::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = \max(Grepper::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? (float) ($rheaders['retry-after']) : 0.0;
        if (\floor($retryAfter) === $retryAfter && $retryAfter <= Grepper::getMaxRetryAfter()) {
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which Grepper use.
